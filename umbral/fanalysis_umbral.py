...
if __name__ == "__main__":
    ...
    for file in os.listdir('matrices_filtradas'):
        file_name = file.split(os.sep)[-1].split('-')[-1].split('.')[0]
        
        print('\n\n' + file_name + '\n')
        if os.path.isfile(f'parametros_matrices_filtradas_umbral/params-{file_name}.txt'):
            continue

        df = cudf.read_csv(
            filepath_or_buffer=f'matrices_filtradas/{file}',
            header=None,
        ...
        def get_params():
            ...
            for percentage in percentages:
                ...
                dataframe.to_csv(
                  path_or_buf=f'parametros_matrices_filtradas_umbral/params-{file_name}.txt',
                  index=False,
                  header=True,
                  sep='\t')
